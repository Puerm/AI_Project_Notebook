...
| app\analyzer\domain_analyzer.py | LLM 业务板块识别（两级：主板块+子板块），v1.0.1 支持 err_info 诊断 | analyze_business_domains, _build_domain_analysis_prompt, _parse_domain_response, _degraded_domain_result | - |
| app\analyzer\llm_assistant.py | LLM API 基础设施（调用/配置/Key检测/tool-use），v1.0.1 _call_llm 返回元组 | _load_dotenv, _get_llm_config, _call_llm, _call_llm_with_tools, check_api_key_available | - |
| app\analyzer\codegraph.py | CodeGraph 集成核心模块：SQLite 符号知识图谱检测、Schema 提取、只读查询 | detect_codegraph_db, connect_codegraph_db, extract_schema_summary, execute_query, format_schema_for_llm | - |
| app\analyzer\digest_collector.py | codebase-digest 集成与维度文件筛选，v1.0.1 token 预算+优先级排序+上限 | is_cdigest_available, collect_digest, preprocess_digest, format_digest_for_llm, format_files_for_llm, estimate_tokens, filter_for_architecture, filter_for_user_stories, filter_for_risk | - |
| app\analyzer\dimension_analyzer.py | 聚焦三维度分析引擎（架构/用户故事/风险），v1.0.1 token 预算+HTTP 400 不重试 | analyze_architecture, analyze_user_stories, analyze_risk, _get_model_context_limit, _compute_token_budget, _call_llm_with_retry | - |
...
| app\analyze_project.py | 智能项目分析引擎入口 v0.5.1 — 渐进式披露 + digest 聚焦三维度分析 + CodeGraph 图谱增强 (--codegraph) | main, _run_codegraph_exploration | - |
