...
COMMANDS = [
    ...
    ("python harness/scripts/diagnose_and_fix.py", "自我升级引擎 — 扫描反馈信号的重复模式，自动诊断并应用修复（沙盒验证）"),
    ("python harness/scripts/harness_deploy.py <目标路径>", "Harness 框架个性化部署 — LLM 检测新项目并生成适配建议，交互式确认"),
    ("", ""),
    ("--- 手工操作 ---", ""),
